...
def ensure_gpu(tensor, device):
    if "cuda" in str(type(tensor)) and device is not None:
        return tensor.cuda(device)
    elif "cuda" in str(type(tensor)):
        return tensor.cpu()
    elif "Tensor" in str(type(tensor)) and device is not None:
        return tensor.cuda(device)
    elif "Tensor" in str(type(tensor)):
        return tensor
    elif type(tensor) is np.ndarray:
        return cudavec(tensor, device=device).data
    else:
        return tensor
...
class FAISSIndex(object):
    ...
    def __init__(self, cell_size=20, nr_cells=1024, K=4, num_lists=32, probes=32, res=None, train=None, device=-1):
        ...
        if self.device is not None:
            self.res.initializeForDevice(self.device)
    ...
    def add(self, other, positions=None, last=None):
        ...
        if positions is not None:
            positions = ensure_gpu(positions, self.device)
